...
export class CompletedProjectsComponent implements OnInit {
  ...
  getALLProProject() {
    this.projectService.getProjectsByCurrentUserCompleted(this.currentUser.email_address).subscribe({
      next: (response: ProjectDetails[] | ProjectDetails) => {
        if (Array.isArray(response)) {
  ...
  getALLTechProject() {
    this.projectService.getProjectsByTechCompleted(this.currentUser.email_address).subscribe({
      next: (response: ProjectDetails[] | ProjectDetails) => {
        if (Array.isArray(response)) {
  ...
  viewProject(project: ProjectDetails) {
    this.router.navigate(['/project/view-completed', project.ptsRef]);
